...
router.route('/auth/auction/:auction_id').post(routes.auths.auctionAuth);

router.route('/auth/admin').post(routes.auths.adminAuth);

router.route('/wimodels/:model').get(routes.wimodels);
...
router.route('/teams/:teamId').get(teamRoutes.getTeam);
...
router.route('/admin').get(middlewares.authenticateAdminFilter);
